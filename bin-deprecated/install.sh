...
apt-get install -y --force-yes --no-install-recommends $apt_deps
...
if [ "$ODOO_VERSION" == "8.0" ]; then
    ...
    apt-get install -y --force-yes --no-install-recommends file
    # Extra dependencies for 'document' (doc & pdf support)
    apt-get install -y --force-yes --no-install-recommends antiword poppler-utils
    # Extra dependencies for Workflow reports
    apt-get install -y --force-yes --no-install-recommends graphviz ghostscript
fi
